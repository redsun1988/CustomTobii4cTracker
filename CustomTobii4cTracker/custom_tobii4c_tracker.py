...
class Tobii4cTracker:
    ...
    def find_center(self) -> Tuple:
        ...
        if hwnd:
            ...
            target_centers = [(c[0], c[1]) for c in colored_centers if c[2] == self.image_processor.target_color]
            target_center = target_centers[0] if len(target_centers) > 0 else (None, None)
